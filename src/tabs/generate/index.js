...
import { useDatasContext, useDatasContextFn } from "../../contexts"
...
import { formatCameraPinDefines } from "../../configuration/cameraPins"
import { formatSerialPinDefines } from "../../configuration/serialPins"
import {
...
const configurationFile = (data) => {
    const snapshot = collectConfigurationSnapshot(data)
    return (
        `// This file was generated by ESP3D-Configurator V${Version} \n` +
        header +
        convertToText(data) +
        formatCameraPinDefines(data, useDatasContextFn.getValueId) +
        formatSerialPinDefines(useDatasContextFn.getValueId) +
        footer +
        formatSnapshotComment(snapshot)
    )
}
...
const libIgnore = ({
    ...
    let ignore = sections[target].lib_ignore || ""
    if (cameraName != "-1" && !ignore.includes("TFT_eSPI")) {
        ignore += ignore ? "\tTFT_eSPI\n" : "TFT_eSPI\n"
    }
    if (displayName != "-1" && ignore.includes("TFT_eSPI")) {
        ignore = ignore.replace(/\t?TFT_eSPI\n?/g, "")
    }
    return ignore ? "lib_ignore = " + ignore + (ignore.endsWith("\n") ? "" : "\n") : ""
}
...
const buildFlags = ({
    ...
    const needsPsram = has_psram || cameraName != "-1"
    return (
        "build_flags = " +
        sections[target].build_flags +
        displayBuildFlags(target, displayName) +
        psramBuildFlags(target, needsPsram) +
...
const partitionScheme = (
    ...
    if (target === "esp8266") {
        ...
        switch (targetsize) {
            ...
            case "4":
                if (cameraName != "-1") {
                    return "board_build.partitions = min_spiffs.csv\n"
                }
...
const convertToText = (data) => {
    ...
    return Object.keys(data).reduce((acc, item) => {
        return data[item].reduce((acc2, item2) => {
            if (item2.type == "group") {
                if (!canshow(item2.depend) || item2.skipExport) return acc2
